...
list, with its real dimensions. \texttt{pageless.ts} compiles the document
...
allows), and \texttt{stack.ts}, which stacks those pages into one.
...
\section*{Making one}
The tool is in \texttt{tools/pageless-pdf/}. It needs LuaTeX and Node,
with the repository's packages installed (\texttt{npm ci}):
{{< /latex >}}

```sh
npm ci
node tools/pageless-pdf/pageless.ts paper.tex -o out/
...
The same directory holds the comparison. \texttt{site-from-run.ts} builds
...
width:
\begin{description}
\item[\texttt{vector-compare.ts}] No pixels: the position of every glyph
  ...
  not rasterisation.
\item[\texttt{compare.ts}] Pixels: both drawn at the same scale, the rows
...
node $T/site-from-run.ts out/ out/site/           # then serve out/site/, e.g. on port 8000
