...
% (src/pipeline/pipeline.ts) substitutes two placeholders, written further down as
...
\def\reflowtex@flowfloat#1{%
  ...
  \begingroup
  \def\@captype{#1}%
  % no indent, as in the box LaTeX sets a float in (\@parboxrestore): a
  % tabular* or picture \linewidth wide fills the line exactly
  \parindent\z@
...
% the viewer to place the aside by. \marginpar (below) and reflowtex.sty's
...
\long\def\reflowtexAside#1#2#3{%
  ...
  \begingroup
    ...
    \insert\reflowtexAsideIns{%
      ...
      \hsize\linewidth \parindent0pt \everypar{}#3\par}%
  \endgroup}
