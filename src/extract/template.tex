...
% moved ahead of the prose that introduced it. Make figures and tables
% ordinary flow containers instead – the starred, full-width forms too: left
% as floats on a page that never fills, they would all wait for the final
% \clearpage and land after the last paragraph. Their captions are ordinary
% paragraphs (rather than the direct boxes LaTeX's float machinery builds,
% which are invisible to the paragraph content stream), while the standard
% counter and \label semantics are retained. Placement options are accepted
% for source compatibility and deliberately ignored.
\makeatletter
\def\reflowtex@flowfloat#1{%
  \par\addvspace{\intextsep}%
  \begingroup
  \def\@captype{#1}%
  % spaced as LaTeX's \@makecaption spaces it: \abovecaptionskip above,
  % \belowcaptionskip below, whatever the class set them to
  \RenewDocumentCommand{\caption}{o m}{%
    \refstepcounter{#1}%
    \par\vskip\abovecaptionskip
    {\small\centering\csname fnum@#1\endcsname: ##2\par}%
    \vskip\belowcaptionskip}}
\def\reflowtex@flowfloatend{\par\endgroup\addvspace{\intextsep}}
\RenewDocumentEnvironment{figure}{O{}}{\reflowtex@flowfloat{figure}}{\reflowtex@flowfloatend}
\@ifundefined{table}{}{%
  \RenewDocumentEnvironment{table}{O{}}{\reflowtex@flowfloat{table}}{\reflowtex@flowfloatend}}
\@ifundefined{figure*}{}{%
  \RenewDocumentEnvironment{figure*}{O{}}{\reflowtex@flowfloat{figure}}{\reflowtex@flowfloatend}}
\@ifundefined{table*}{}{%
  \RenewDocumentEnvironment{table*}{O{}}{\reflowtex@flowfloat{table}}{\reflowtex@flowfloatend}}
...
% capture below) – whether or not hyperref is loaded, whose \contentsline
% takes a fourth argument (its own anchor name), passed on untouched.
...
\def\reflowtex@newcontentsline#1#2#3{%
  \@ifnextchar\bgroup{\reflowtex@contentsline{#1}{#2}}{\reflowtex@contentsline{#1}{#2}{}}}
\def\reflowtex@contentsline#1#2#3{%
  ...
  \else
    \reflowtex@origcontentsline{#1}{#2}{#3}%
  \fi}
% the original, with the page number left out (and hyperref's anchor, when
% it takes one)
\def\reflowtex@origcontentsline#1#2#3{%
  \ifreflowtex@hyperref \reflowtexOrigContentsline{#1}{#2}{}{#3}%
  \else \reflowtexOrigContentsline{#1}{#2}{}\fi}
\newif\ifreflowtex@hyperref
\AtBeginDocument{%
  \@ifpackageloaded{hyperref}{\reflowtex@hyperreftrue}{}%
