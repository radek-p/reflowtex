...
for (const [fn, run] of cases) {
  ...
  test(`${fn}: ${files.length} captured call(s) replayed`, { skip: files.length ? false : 'no captures (tests/parity/README.md)' }, async () => {
    ...
    for (const f of files) {
      ...
      const docs = (a: any[]) => a.filter(x => x && typeof x === 'object').map(asDocument);
      assert.deepStrictEqual(normalise(Array.isArray(result) ? result.map(asDocument) : result), normalise(Array.isArray(rec.result) ? rec.result.map(asDocument) : rec.result), `${f}: result`);
      if (rec.after) assert.deepStrictEqual(normalise(docs(rec.args)), normalise(docs(rec.after)), `${f}: arguments after the call`);
      n++;
    }
    assert.ok(n > 0);
  });
