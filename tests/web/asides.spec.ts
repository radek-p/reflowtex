...
test('query, width, render, anchor', async ({ openPage }) => {
  ...
  const r = await page.evaluate(() => {
    const h = reflowtex.host, all = h.instances({ placement: 'detached' }), plain = h.instances({ kind: 'plain', for: 'y' });
    const el = document.createElement('div'); document.body.append(el);
    const body = plain[0].part('body'), m = body.mount(el, { width: 'natural' }).metrics();
    const a = plain[0].anchor();
    return { kinds: all.map((a: any) => a.kind).sort(), plain: plain.length, width: body.naturalWidth() as number,
             drawn: { width: m.width, baseline: m.firstBaseline },
             anchor: a && [a.x, a.y], block: plain[0].block.el === document.querySelectorAll('.latex-block[data-nodelist-b64]')[1] };
  });
...
test('anchor on its line', async ({ openPage }) => {
  const page = await openPage('asides');
  const r = await page.evaluate(() => {
    const a = reflowtex.host.instances({ kind: 'plain' })[0], at = a.anchor();
    const lines = [...a.block.el.querySelectorAll('svg text tspan')].map((t: any) => {
...
test('side notes in the margin', async ({ openPage }) => {
  ...
  const anchors: number[] = await page.evaluate(() => reflowtex.host.instances({ place: 'margin' }).map((a: any) => a.anchor().y));
